...
const DemoSignin = () => {
  ...
  const onSubmitHandler: SubmitHandler<SigninForm> = async (data, event) => {
    const res = await signin({studentid: data.studentid, password: data.password, publicKey: rsaKey})
    if (res.error) {
      setError("password", {message: res.error.customError ? res.error.customError : JSON.stringify(res.error.zodError)})
    }
  }
      ...
      <form onSubmit={handleSubmit(onSubmitHandler)} className={cn("border rounded border-black p-2")}>
        <Controller
          ...
          render={({field, fieldState: {error}}) => {
            return (
              <div>
                <Input className={cn("border rounded border-black px-2")} {...field}/>
        ...
        <Controller
          ...
          render={({field, fieldState: {error}}) => {
            return (
              <div>
                <Input type={"password"} className={cn("border rounded border-black px-2")} {...field}/>
                <ErrorMessage>{error ? error.message : ""}</ErrorMessage>
